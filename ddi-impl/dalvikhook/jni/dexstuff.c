...
{
	d->dvm_hand = dlopen("libdvm.so", RTLD_LAZY| RTLD_GLOBAL);
	log("dvm_hand = 0x%x\n", d->dvm_hand)

	if (d->dvm_hand) {
        d->dvmFindClassNoInit_fnPtr = mydlsym(d->dvm_hand,"_Z12dvmFindClassPKcP6Obje");
        ...
        d->dvmFindSystemClassNoInit_fnPtr = mydlsym(d->dvm_hand, "_Z24dvmFindSystemClassNoI");

        d->dvmFindClassNoInit_fnPtr = mydlsym(d->dvm_hand,"dvmFindClass");
		d->gDvm = mydlsym(d->dvm_hand, "gDvm");
	}
	void * ssl_hand = dlopen("libssl.so", RTLD_LAZY| RTLD_GLOBAL);
}
