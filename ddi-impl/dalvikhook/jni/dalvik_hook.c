...
void* dalvik_hook(struct dexstuff_t *dex, struct dalvik_hook_t *h)
{
    ...
    if(target_cls == NULL){
        ...
        log("method\n");
        target_cls = dex->dvmFindClassNoInit_fnPtr(h->clname, m->clazz->classLoader);
        if(target_cls == NULL)
            log("dvmFindClassInit: Can not Find class %s\n", h->clname);
        log("dvmFindClassInit: Find class %s\n", h->clname);
    }


	if (h->debug_me)
		log("class %s = 0x%x\n", h->clname, (unsigned int)target_cls)
    ...
    h->orign_method =(Method*)malloc(sizeof(Method));
    memcpy(h->orign_method, h->method,sizeof(Method));
	// constrcutor workaround, see "dalvik_prepare" below
	if (!h->resolvm) {
		h->cls = target_cls;
		h->mid = (void*)h->method;
		h->orign_mid = (void*)h->orign_method;
	}

	if (h->debug_me)
		log("%s(%s) = 0x%x, ox%x\n", h->method_name, h->method_sig, (unsigned int)h->method,(unsigned int)h->orign_method);

	if (h->method) {
		h->insns = h->method->insns;

		if (h->debug_me) {
			log("nativeFunc %x\n", (unsigned int)h->method->nativeFunc)
		...
		if (h->debug_me)
			log("patched %s to: 0x%x\n", h->method_name,(unsigned int) h->native_func)
