...
//        fprintf(stderr, "save_cache filelocation: %s\n",file_location);
        if(strcmp(delta_stat->items[i].file_id,id) == 0){
            ...
            if(it->cache != NULL && header_size != 0){
                free(it->cache);
                it->cache = NULL;
                it->cache_size = 0;
                it->body_size = 0;
                ...
                //appending data
                memcpy(it->cache + it->recv_size, data, cur_recv);
                it->recv_size += cur_recv;
                return i;
            }
            delta_stat->items[i].cache = malloc(header_size + body_size);
            memcpy(delta_stat->items[i].cache + delta_stat->items[i].recv_size, data, cur_recv);
            delta_stat->items[i].cache_size = header_size + body_size;
            delta_stat->items[i].body_size = body_size;
            delta_stat->items[i].recv_size = cur_recv;
            /* log("cache saved i %zu\n",i); */

//            fprintf(stderr,"in save_cache 2 %s %zu,%s \n",delta_stat->items[i].file_id,  delta_stat->items[i].cache_size,delta_stat->items[i].cache);
            return i;
        }
    }
    return -1;
...
"GET /v1.0/drive/items/?expand=children(expand%3Dthumbnails)%2Cthumbnails HTTP/1.1\r\n"//select=id,createdDateTime,cTag,eTag,lastModifiedDateTime,name,parentReference,size,file HTTP/1.1\r\n"
"X-RequestStats: SDK-Version=Android-v1.1.5\r\n"
"Authorization: bearer \r\n"
"User-Agent: Dalvik/1.6.0 (Linux; U; Android 4.4.4; GT-I9500 Build/KTU84Q)\r\n"
"Host: api.onedrive.com\r\n"
...
char* generate_md_request(char* id, char* bearer_token,size_t s)
{
    char* request;
    size_t size, pos;
